...
private object shared {
    ...
    val arbeidsgiver: ArbeidsgiverInfo = ArbeidsgiverInfo.Flere(
        "Standard AS",
        "Utvikler",
        100,
    ...
    val behandler: Behandler = Behandler(
...
object legacy {
    val metadata = SykmeldingMeta.Legacy(
    ...
    val medisinskVUrdering = MedisinskVurdering.Legacy(
        hovedDiagnose = shared.hovedDiagnose,
        biDiagnoser = listOf(
            shared.hovedDiagnose,
            DiagnoseInfo(DiagnoseSystem.ICD10, "A500", "Tidlig medfødt syfilis,\nsymptomgivende")
        ),
        svangerskap = true,
...
object utenlandsk {
    val sykmelding = Sykmelding.Utenlandsk(
    ...
    val record = SykmeldingRecord.Utenlandsk(
...
object papir {
    val sykmelding = Sykmelding.Papir(
        id = shared.sykmeldingId,
        pasient = shared.pasient,
        medisinskVurdering = legacy.medisinskVUrdering,
        aktivitet = shared.aktivitet,
    ...
    val record = SykmeldingRecord.Papir(
...
object xml {
    ...
    val record = SykmeldingRecord.Xml(
        metadata = MessageMetadata.Xml.Emottak.Legacy(
            MessageInfo(
                Meldingstype.SYKMELDING,
                genDate = OffsetDateTime.now(ZoneOffset.UTC),
                msgId = UUID.randomUUID().toString(),
...
object digital {
    val sykmelding = Sykmelding.Digital(
        id = shared.sykmeldingId,
        ...
        medisinskVurdering = MedisinskVurdering.Digital(
            hovedDiagnose = shared.hovedDiagnose,
            biDiagnoser = listOf(
                shared.hovedDiagnose, DiagnoseInfo(
    ...
    val record = SykmeldingRecord.Digital(
