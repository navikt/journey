...
fun mapArbeidsgiver(arbeidsgiver: ArbeidsgiverInfo, metadataType: MetadataType): Arbeidsgiver {
    ...
    val stillingsprosent =
        (arbeidsgiver as? ArbeidsgiverInfo.En)?.stillingsprosent
...
private fun MedisinskArsakType.label(): String =
    when (this) {
        ...
        is Aktivitet.IkkeMulig ->
            AktivitetRad(
                ...
                medisinskArsak =
                    medisinskArsak?.let {
                        AktivitetArsak(
                            arsaker = it.arsak.map(MedisinskArsakType::label),
                ...
                arbeidsrelatertArsak =
                    arbeidsrelatertArsak?.let {
                        AktivitetArsak(
                            arsaker = it.arsak.map(ArbeidsrelatertArsakType::label),
                            beskrivelse = it.beskrivelse,
                        )
                    },
            )
...
private val AKTIVITET_REKKEFOLGE =
    listOf(
        "AVVENTENDE",
        "GRADERT",
        "AKTIVITET_IKKE_MULIG",
...
private val ArbeidsgiverInfo.meldingTilArbeidsgiver: String?
    get() =
        (this as? ArbeidsgiverInfo.En)?.meldingTilArbeidsgiver
...
fun mapMeldingTilArbeidsgiver(
    arbeidsgiver: ArbeidsgiverInfo,
    regelsettVersjon: String?,
...
private fun List<PersonId>.hpr(): String? = firstOrNull { it.type == PersonIdType.HPR }?.id
...
private fun MessageMetadata.senderNavn(): String? =
    when (this) {
...
fun mapAvvisning(validation: ValidationResult): List<AvvisningRad> {
    ...
    return validation.rules.filterIsInstance<Rule.Invalid>().map {
        AvvisningRad(sykmeldt = it.reason.sykmeldt, sykmelder = it.reason.sykmelder)
    }
}

fun mapPrognose(
    prognose: no.nav.tsm.sykmelding.input.core.model.Prognose?,
    regelsettVersjon: String?,
): Prognose? {
    ...
    val arbeid =
        prognose.arbeid
            ?.takeIf { regelsettVersjon != "3" }
            ?.let { a ->
                when (a) {
                    is IArbeid.ErIArbeid ->
                        PrognoseArbeid(
                            type = a.type.name,
...
private fun utdypendeTittel(nokkel: String): String =
    when (nokkel) {
...
fun mapUtdypendeLegacy(utdypende: Map<String, Map<String, SporsmalSvar>>?): List<UtdypendeGruppe> =
    utdypende.orEmpty().map { (nokkel, sporsmal) ->
        UtdypendeGruppe(
            ...
            sporsmal =
                sporsmal.values.map {
                    UtdypendeSporsmalRad(sporsmal = it.sporsmal, svar = it.svar)
                },
        )
    }
...
private fun spmMapping(prefix: String): Map<Sporsmalstype, Pair<String, String>> =
    mapOf(
